...
def move(s, e):
    ...
    if move.isLegal(s, globals.BOARD) and not move.check(s, e, globals.BOARD):
        ...
        m = {
            ...
            'endSq': move.endSq,
            'movedFig': move.pieceMoved,
            'beatenFig': move.pieceCaptured,
        }
        moveLog.append(m)
    else:
        print('invalid')
...
def undoMove():
    if len(moveLog) > 0:  # Ensure there is at least one move to undo
        ...
        globals.BOARD[move['startSq'][0]][move['startSq'][1]] = move['movedFig']
        globals.BOARD[move['endSq'][0]][move['endSq'][1]] = move['beatenFig']
        globals.TURN -= 1
